...
class SiameseHelper(object):
    ...
    @staticmethod
    def _get_single_selected_blob(adapter, layer_def, siamese_view_mode, blob_selector):
        '''
        ...
        if layer_def['format'] == 'normal':
            if blob_selector == 'data':
                return adapter.get_layer_data(layer_def['name/s'])[0]
            elif blob_selector == 'diff':
                return adapter.get_layer_diff(layer_def['name/s'])[0]
            else:
                assert False, 'invalid blob_selector'
            ...
            if blob_selector == 'data':
                return adapter.get_layer_data(selected_layer_name)[0]
            elif blob_selector == 'diff':
                return adapter.get_layer_diff(selected_layer_name)[0]
            else:
                assert False, 'invalid blob_selector'
            ...
            if blob_selector == 'data':
                return adapter.get_layer_data(layer_def['name/s'])[selected_batch_index]
            elif blob_selector == 'diff':
                return adapter.get_layer_diff(layer_def['name/s'])[selected_batch_index]
            else:
                assert False, 'invalid blob_selector'
    ...
    @staticmethod
    def get_single_selected_data_blob(adapter, layer_def, siamese_view_mode):
        '''
        ...
        return SiameseHelper._get_single_selected_blob(adapter, layer_def, siamese_view_mode, blob_selector='data')

    @staticmethod
    def get_single_selected_diff_blob(adapter, layer_def, siamese_view_mode):
        '''
        ...
        return SiameseHelper._get_single_selected_blob(adapter, layer_def, siamese_view_mode, blob_selector='diff')

    @staticmethod
    def _get_siamese_selected_blobs(adapter, layer_def, siamese_view_mode, blob_selector):
        '''
        ...
        if layer_def['format'] == 'normal':
            ...
            if blob_selector == 'data':
                return adapter.get_layer_data(layer_def['name/s'][0])[0], \
                       adapter.get_layer_data(layer_def['name/s'][1])[0]
            elif blob_selector == 'diff':
                return adapter.get_layer_diff(layer_def['name/s'][0])[0], \
                       adapter.get_layer_diff(layer_def['name/s'][1])[0]
            else:
                assert False, 'invalid blob_selector'
            ...
            if blob_selector == 'data':
                return adapter.get_layer_data(layer_def['name/s'])[0], \
                       adapter.get_layer_data(layer_def['name/s'])[1]
            elif blob_selector == 'diff':
                return adapter.get_layer_diff(layer_def['name/s'])[0], \
                       adapter.get_layer_diff(layer_def['name/s'])[1]
            else:
                assert False, ''
    ...
    @staticmethod
    def get_siamese_selected_data_blobs(adapter, layer_def, siamese_view_mode):
        '''
        ...
        return SiameseHelper._get_siamese_selected_blobs(adapter, layer_def, siamese_view_mode, blob_selector='data')

    @staticmethod
    def get_siamese_selected_diff_blobs(adapter, layer_def, siamese_view_mode):
        '''
        ...
        return SiameseHelper._get_siamese_selected_blobs(adapter, layer_def, siamese_view_mode, blob_selector='diff')
    ...
    @staticmethod
    def backward_from_layer(adapter, backprop_layer_def, backprop_unit, siamese_view_mode):
        ...
        if (backprop_layer_def['format'] == 'siamese_batch_pair') or \
            ...
            diffs0, diffs1 = SiameseHelper.get_siamese_selected_diff_blobs(adapter, backprop_layer_def, siamese_view_mode)
            diffs0, diffs1 = diffs0 * 0, diffs1 * 0
            data0, data1 = SiameseHelper.get_siamese_selected_data_blobs(adapter, backprop_layer_def, siamese_view_mode)
            ...
            if backprop_layer_def['format'] == 'siamese_layer_pair':
                adapter.backward_from_layer(backprop_layer_def['name/s'][0], diffs0, zero_higher=True)
                adapter.backward_from_layer(backprop_layer_def['name/s'][1], diffs1, zero_higher=True)
                ...
                adapter.backward_from_layer(backprop_layer_def['name/s'], diffs, zero_higher=True)
        else:

            diffs = SiameseHelper.get_single_selected_diff_blob(adapter, backprop_layer_def, siamese_view_mode)
            diffs = diffs * 0
            data = SiameseHelper.get_single_selected_data_blob(adapter, backprop_layer_def, siamese_view_mode)
            ...
            adapter.backward_from_layer(selected_backprop_layer_name, diffs, zero_higher=True)

        pass

    @staticmethod
    def deconv_from_layer(adapter, backprop_layer_def, backprop_unit, siamese_view_mode, deconv_type):
        ...
        if (backprop_layer_def['format'] == 'siamese_batch_pair') or \
            ...
            if backprop_layer_def['format'] == 'siamese_layer_pair':
                adapter.deconv_from_layer(backprop_layer_def['name/s'][0], diffs0, zero_higher=True, deconv_type=deconv_type)
                adapter.deconv_from_layer(backprop_layer_def['name/s'][1], diffs1, zero_higher=True, deconv_type=deconv_type)

            elif backprop_layer_def['format'] == 'siamese_batch_pair':
                # combine them to 2-batch and send once
                diffs = concatenate((diffs0, diffs1), axis=0)

                adapter.deconv_from_layer(backprop_layer_def['name/s'], diffs, zero_higher=True, deconv_type=deconv_type)
            ...
            adapter.deconv_from_layer(selected_backprop_layer_name, diffs, zero_higher=True, deconv_type=deconv_type)
    ...
    @staticmethod
    def get_layer_output_size(adapter, is_siamese, layer_def, siamese_view_mode):
        ...
        if (layer_def['format'] == 'siamese_batch_pair') or (layer_def['format'] == 'siamese_layer_pair' and siamese_view_mode == SiameseViewMode.BOTH_IMAGES):
            ...
            data0, data1 = SiameseHelper.get_siamese_selected_data_blobs(adapter, layer_def, siamese_view_mode)
            return data0.shape

        else:

            data = SiameseHelper.get_single_selected_data_blob(adapter, layer_def, siamese_view_mode)
            return data.shape
