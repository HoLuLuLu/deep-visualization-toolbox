...
def main():
    ...
    parser.add_argument('--gpu', action='store_true',
                        default=settings.caffevis_mode_gpu, help='Use gpu.')
    parser.add_argument('--do-maxes', action='store_true',
                        default=settings.max_tracker_do_maxes,
                        help='Output max patches.')
    parser.add_argument('--do-deconv', action='store_true',
                        default=settings.max_tracker_do_deconv,
                        help='Output deconv patches.')
    parser.add_argument('--do-deconv-norm', action='store_true',
                        default=settings.max_tracker_do_deconv_norm,
                        help='Output deconv-norm patches.')
    parser.add_argument('--do-backprop', action='store_true',
                        default=settings.max_tracker_do_backprop,
                        help='Output backprop patches.')
    parser.add_argument('--do-backprop-norm', action='store_true',
                        default=settings.max_tracker_do_backprop_norm,
                        help='Output backprop-norm patches.')
    parser.add_argument('--do-info', action='store_true',
                        default=settings.max_tracker_do_info,
                        help='Output info file containing max filenames and labels.')
    parser.add_argument('--idx-begin', type=int, default=None,
    ...
    parser.add_argument('--nmt_pkl', type=str,
                        default=os.path.join(settings.caffevis_outputs_dir,
                                             'find_max_acts_output.pickled'),
                        help='Which pickled NetMaxTracker to load.')
    parser.add_argument('--net_prototxt', type=str,
    ...
    parser.add_argument('--datadir', type=str,
                        default=settings.static_files_dir,
                        help='directory to look for files in')
    parser.add_argument('--filelist', type=str,
                        default=settings.static_files_input_file,
    ...
    parser.add_argument('--outdir', type=str,
                        ...
                        help='Which output directory to use. Files are output into outdir/layer/unit_%%04d/{maxes,deconv,backprop}_%%03d.png')
    parser.add_argument('--search-min', action='store_true', default=False,
                        help='Should we also search for minimal activations?')
    ...
    for layer_name in settings.layers_to_output_in_offline_scripts:
        pool.apply(processBatch, (
        layer_name, SiameseHelper(settings.layers_list),
        load_max_tracker_from_file(args.nmt_pkl), args, net))


def processBatch(thread_layer_name, thread_siamese_helper, thread_nmt,
                 thread_args, thread_net):
    print 'Started work on layer %s' % (thread_layer_name)

    normalized_layer_name = thread_siamese_helper.normalize_layer_name_for_max_tracker(
        thread_layer_name)

    mt = thread_nmt.max_trackers[normalized_layer_name]

    if thread_args.idx_begin is None:
        idx_begin = 0
    if thread_args.idx_end is None:
        idx_end = mt.max_vals.shape[0]

    with WithTimer('Saved %d images per unit for %s units %d:%d.' % (
            thread_args.N, normalized_layer_name, idx_begin, idx_end)):

        output_max_patches(settings, mt, thread_net, normalized_layer_name,
                           idx_begin,
                           idx_end,
                           thread_args.N, thread_args.datadir,
                           thread_args.filelist, thread_args.outdir,
                           False,
                           (thread_args.do_maxes, thread_args.do_deconv,
                            thread_args.do_deconv_norm,
                            thread_args.do_backprop,
                            thread_args.do_backprop_norm,
                            thread_args.do_info))

        if thread_args.search_min:
            output_max_patches(settings, mt, thread_net, normalized_layer_name,
                               idx_begin, idx_end,
                               thread_args.N, thread_args.datadir,
                               thread_args.filelist, thread_args.outdir,
